...
class HostedEngineSetup extends Component {
	...
	handleOvirtHostedEngineCleanupSelection(startCleanup) {
		if (startCleanup) {
			this.setState({ isCleanupRunning: true });
			cockpit
				.spawn(["ovirt-hosted-engine-cleanup", "-q"])
				.done((std) => {
					console.log(std);
					this.setState({ isCleanupRunning: false });
				})
				.fail((error) => {
					console.log(error);
				});
		}
		this.setState({ state: heSetupState.EMPTY, closeRequestRecvd: false });
	}
			...
			<div>
				...
				{this.state.state === heSetupState.EMPTY && (
					<Curtains
						...
						isCleanupRunning={this.state.isCleanupRunning}
					/>
				)}
						...
						{this.state.closeRequestRecvd && (
							<span>
								<CloseWizardConfirmationDialog
									...
									startOvirtHostedEngineCleanup={
										this.handleOvirtHostedEngineCleanupSelection
									}
									cancelled={this.state.cancelled}
								/>
							</span>
...
const DeploymentOptionPanel = ({
	...
	let startDeployButton = isCleanupRunning ? (
		<button
			id={id}
			className="btn btn-primary"
			disabled={isCleanupRunning}
			title="oVirt Hosted Engine Cleanup is still running"
...
class CloseWizardConfirmationDialog extends Component {
	...
	render() {
		...
		if (this.props.cancelled) {
			exitDialog = (
				<div className="popup-dialog-text">
					Activate oVirt Hosted Engine cleanup tool. Caution, this may cause
					network connection lose. Proceed?
				</div>
			);
			...
			yesButton = (
				<button
					type="button"
					className="btn btn-default confirm-close-dialog-btn"
					aria-label="Start Ovirt Hosted Engine Cleanup"
					onClick={(e) => this.props.startOvirtHostedEngineCleanup(true)}
				>
					Yes
				</button>
			);
			...
			noButton = (
				<button
					...
					className="btn btn-default confirm-close-dialog-btn"
					style={{ float: "right" }}
					aria-label="Skip Ovirt Hosted Engine Cleanup"
					onClick={(e) => this.props.startOvirtHostedEngineCleanup(false)}
				>
					No
				</button>
			);
				...
				<div className="modal-dialog modal-lg" style={{ width: "400px" }}>
					<div className="modal-content">
						...
						<div className="modal-body clearfix">
							<div>
								<div className="row">
									<div className="col-sm-10 col-sm-offset-1">{exitDialog}</div>
								</div>

								<div className="row popup-dialog-btn-row">
									<div className="col-sm-3 col-sm-offset-3">{yesButton}</div>

									<div className="col-sm-3">{noButton}</div>
								</div>
							</div>
						</div>
					</div>
