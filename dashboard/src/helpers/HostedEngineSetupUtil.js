...
import { CONFIG_FILES as gdeployConstants } from '../components/gdeploy/constants'
import AnsibleVarFilesGenerator from "./HostedEngineSetup/AnsibleVarFilesGenerator"
...
export class PlayBookExecutor {
  runPlaybookToAddHost() {
      const playBook = configValues.ADD_HOST_PLAYBOOK_PATH;
      const hostPath = gdeployConstants.ansibleHostFile;
      const self = this;
      const varFileGenerator = new AnsibleVarFilesGenerator(this.heSetupModel);
      return new Promise((resolve, reject) => {
        self.isGlusterServiceEnabled(function(isGSEnabled) {
          if(isGSEnabled) {
            cockpit.file(hostPath).read()
                .done(function (content, tag) {
                  if (content != null && tag != "-") {
                    let hostArr = content.split("\n");
                    varFileGenerator.readAnswerFile(function(result) {
                          if (result.length != 0) {
                            hostArr.forEach(function(eachHost, hIndex) {
                              let cmd = "ansible-playbook -i " + hostPath + " " + playBook + " --extra-vars " + '"' +
                                        "target=all FQDN=" + result[0] + " ADMIN_PASSWORD=" + result[1] + " HOST_NAME=" + eachHost +
                                        " HOST_ADDRESS=" + eachHost + '"' + " --limit " + eachHost;
                                console.log(cmd);
                                ...
                                $(self.channel).on("close", function(ev, options) {
                                    let denied = false;
                                    if (!self._manual_close) {
                                        if (options["problem"] === "access-denied") {
                                            ...
                                            console.log("Execution of " + playBook + " completed successfully.");
                                            resolve();
                                        } else {
                                            console.log("Execution of " + playBook + " failed to complete.");
                                            reject(options, denied);
                                        }
                                    }
                                });
                          ...
                          console.log("Failed to get answer file content.");
                        }
                    });
                } else {
                  console.log("Either deployment is not gluster based or there is a problem to get host details.");
                }
            });
          }
        });
  ...
  runPlaybookToCreateSD() {
      const playBook = configValues.ADD_SD_PLAYBOOK_PATH;
      const hostPath = gdeployConstants.ansibleHostFile;
      const self = this;
      const varFileGenerator = new AnsibleVarFilesGenerator(this.heSetupModel);
      return new Promise((resolve, reject) => {
          ...
          if(isGSEnabled) {
            cockpit.file(hostPath).read()
                .done(function (content, tag) {
                  if (content != null && tag != "-") {
                    let hostArr = content.split("\n");
                    self.getGlusterModel(function(glusterModel) {
                      let volumes = glusterModel.volumes
                              ...
                              if (result.length != 0) {
                                for(let i=1; i<volumes.length; i++) {
                                  hostArr.forEach(function(eachHost, hIndex) {
                                    let cmd = "ansible-playbook -i " + hostPath + " " + playBook + " --extra-vars " + '"' +
                                              "target=all FQDN=" + result[0] + " ADMIN_PASSWORD=" + result[1] + " HOST_NAME=" + eachHost +
                                              " HOST_ADDRESS=" + eachHost + " STORAGE_DOMAIN_NAME=" + volumes[i].name + " STORAGE_DOMAIN_PATH=" +
                                              glusterModel.hosts[0] + ":/" + volumes[i].name + "STORAGE_DOMAIN_ADDR=" + eachHost + " MOUNT_OPTIONS=backup-volfile-servers=" +
                                              hostArr[0] + ":" + hostArr[1] + '"' + " --limit " + eachHost;
                                      console.log(cmd);
                                      self.channel = cockpit.channel({
                                          "payload": "stream",
                                          "environ": [
  ...
  getGlusterModel(callback) {
    const filePath = gdeployConstants.gdeployJsonFile;
    cockpit.file(filePath, {"superuser":"require", syntax: JSON }).read()
    .done(function(content, tag) {
      if(content != null) {
        callback(content)
      } else {
  ...
  isGlusterServiceEnabled(callback) {
    cockpit.spawn(
      [ "systemctl", "status", "glusterd" ], { "superuser":"require" }
    ).done(function(code){
      callback(true)
    }).fail(function(code){
      callback(false)
