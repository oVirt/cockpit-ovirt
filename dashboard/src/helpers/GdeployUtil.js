...
var GdeployUtil = {
    ...
    createGdeployConfig(glusterModel, templateModel, filePath, gdeployWizardType = "none", callback) {
        ...
        const volumeConfigs = this.createVolumeConfigs(glusterModel, volumeTemplate)
    ...
    createPreFlightCheck(hosts, pvConfig) {
        ...
        pvConfig.forEach(function(pvHost, index) {
          if(pvHost.host.length > 0){
            ...
            hostPreFlightCheckObject.hostPreFlightCheck.file = `${PRE_FLIGHT_CHECK_SCRIPT} -d ${disks} -h ${hosts.filter(Boolean).join(', ')}`
    ...
    createVolumeConfigs(glusterModel, volumeTemplate) {
      let volumesList = glusterModel.volumes
      let hostList = glusterModel.hosts
        const volumeConfigs = []
        volumesList.map(function(volume) {
            ...
            if(glusterModel.isSingleNode) {
              config.brick_dirs = `${hostList[0]}:${volume.brick_dir}`
              delete config['replica']
              delete config['replica_count']
              config['key'] = "storage.owner-uid,storage.owner-gid,features.shard,performance.low-prio-threads,performance.strict-o-direct,network.remote-dio,network.ping-timeout,user.cifs,nfs.disable,performance.quick-read,performance.read-ahead,performance.io-cache,cluster.eager-lock"
              config['value'] = "36,36,on,32,on,off,30,off,on,off,off,off,enable"
            } else{
