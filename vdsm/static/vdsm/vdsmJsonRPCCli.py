...
def build_result(code, message, content=None):
    logger.debug(
        'build_result, code: {0}, message: "{1}", content: {2}'.format(
            code, message, content is not None
        )
    )
    result = {'status': {
        'code': code,
...
def http_call(url, method, headers={'Accept': 'application/json'},
              user=None, pwd=None, body=None, verify=False):
    logger.debug(
        "http_call: '{0}',\n  method:{1}, user:{2}, headers:{3}".format(
            url, method, user, headers
        )
    )
    ...
    if method == 'POST':
        result = requests.post(
            url, auth=auth, headers=headers, verify=verify, data=body
        )
...
def get_engine_token():
    ...
    resp = http_call(
        url=url, method='GET', user=credentials['user'], pwd=credentials['pwd']
    )
...
def get_all_vms():
    ...
    resp = http_call(
        url=url,
        method='GET',
        headers=get_default_headers(credentials)
    )
    if resp.status_code == 200:
        content = json.loads(resp.text)
...
def get_running_host(credentials=None):
    ...
    if resp.status_code == 200:
        ...
        return build_result(
            1,
            'Not found',
            "VDSM ID '{0}' not found in engine's hosts list".format(vdsmId)
        )
    return build_result(resp.status_code, resp.text)
...
def run_engine_vm(vmId):
    ...
    resp = http_call(
        url=url,
        method='POST',
        headers=get_default_post_headers(credentials),
        body=xml_request
    )
    # TODO: refactor error handling and returned data
    #       (XML vs. JSON, prune data)
    if resp.status_code == 200:
...
def host_to_maintenance():
    ...
    url = "{0}/api/hosts/{1}/deactivate".format(
        credentials['url'],
        host['content']['id']
    )
    resp = http_call(
...
def fake_vm(src, offset):
    ...
    logger.debug(
        "Fake VM created: vmName: '{0}', vmId: '{1}', srcVmId: '{2}'".format(
            fake['vmName'],
            fake['vmId'],
            src['vmId']
        )
...
def parse_args(service):
    parser = argparse.ArgumentParser(
        description=(
            'Support utility for Cockpit oVirt plugin to invoke '
            'VDSM JSON RPC or Engine REST API.\n'
        )
    )
    parser.add_argument(
        'vdsmCommand',
        help='VDSM command to be invoked',
        choices=[
            'getAllVmStats',
            'shutdown',
            'destroy',
            ...
            'ping',
            'engineBridge',
            'getAllVmStatsFakeExtend',
            'getVdsCapabilities'
        ]
    )
    parser.add_argument(
        'vdsmCommandArgs',
        help='VDSM command arguments',
        nargs='*'
    )
